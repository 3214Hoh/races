...
game = True
FPS = 60
finicsh = Falsefrom pygame import *
from random import randint
...
while game:
    for i in event.get():
        ...
        if i.type == MOUSEBUTTONDOWN and i.button == 1:
            ...
            if card_level_ezze.collidepoint(x, y) and not card_level_normal.collidepoint(x, y) and not card_level_hard.collidepoint(x, y):
                card_level_ezze = Label(0, 440, 150, 60, GREE)

            if card_level_normal.collidepoint(x, y) and not card_level_hard.collidepoint(x, y) and not card_level_ezze.collidepoint(x, y):
                card_level_normal = Label(150, 440, 150, 60, GREE)

            if card_level_hard.collidepoint(x, y) and not card_level_ezze.collidepoint(x, y) and not card_level_normal.collidepoint(x, y):
